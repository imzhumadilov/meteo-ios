...
В `Fast` и `Full` наборы помечаются пропущенными. Отобрать в плане одни лишь
контрактные тесты не вышло: для наборов Swift Testing Xcode не применяет
ни `skippedTests`, ни `selectedTests` — прогон с отбором молча выполняет ноль
тестов и рапортует успех. Поэтому `Contract` гоняет заодно юнит и интеграционные;
на полсекунды это дешевле, чем неверный зелёный.
