...
удалён вместе с остальными шаблонными тестами.

**UI-тест один и только на критический путь.** Много UI-тестов — это медленный
и ненадёжный прогон, который в итоге перестают запускать.

Приложение умеет стартовать на подставных источниках: аргумент запуска
`-ui-testing` заставляет `DIContainer` собрать граф на заглушках вместо сети.
`Meteo/App/UITestingEnvironment.swift` — **единственное место, где продакшн-код
знает о тестах**, и таким оно должно остаться.

Элементы в UI-тесте ищутся только по `accessibilityIdentifier`. Поиск по видимому
тексту ломается при смене формулировок и не переживает локализацию.
...
Сравнение, запись эталонов и артефакты расхождений — **`swift-snapshot-testing`**
от Point-Free, подключён через SPM. Версия зафиксирована в
`Meteo.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved`.

**Отрисовка своя**, а не библиотечная, и на то есть измеренная причина:
библиотека не замораживает анимации, и состояния `loading` и `searching`
расходятся от прогона к прогону — индикатор загрузки попадает в кадр
под случайным углом. Отрисовка в `Snapshot.render` останавливает тайминг
слоёв (`speed = 0`) и снимает кадр через `layer.render(in:)`.

Цена своей отрисовки: системные материалы в снимок не попадают — стеклянная
подложка у кнопки «плюс» на эталоне отсутствует, хотя в приложении она есть.
На сверку вёрстки это не влияет, потому что эталон и снимок делаются одинаково.

Эталоны лежат в `MeteoTests/__Snapshots__/<Набор>/<экран>.<состояние>.png`
и закоммичены. Имя склеивается библиотекой из двух частей, поэтому в тесте
они и передаются отдельно: `screen: "search", state: "hint"`. Одним куском
имя превратилось бы в скрытый файл с точкой в начале.

Перегенерация при **осознанном** изменении вёрстки:
...
Эталоны исключены из ресурсов тестового таргета через `membershipExceptions`
в `project.pbxproj`: синхронизированные папки иначе тащат все `.png` в бандл,
хотя тесты читают их из исходников по `#filePath`. Исключения перечислены
**поимённо** — исключить папку целиком Xcode не умеет, проверено.

Добавили снапшот — допишите строку туда же. Забыть не страшно: лишний файл
уедет в бандл мёртвым грузом, но ничего не сломает.

Разошедшийся снимок библиотека кладёт во временную папку и печатает путь
в сообщении о падении; каталог можно задать переменной `SNAPSHOT_ARTIFACTS`.
Рядом с эталонами он не появляется, коммитить нечего.
...
**Контрактные тесты** ходят в **живой** Open-Meteo и отвечают на один вопрос:
не изменилась ли форма ответа? Интеграционные тесты работают на сохранённых
фикстурах, и в этом их слепое пятно — если API сменит формат, они останутся
зелёными, а приложение сломается. Фикстура это снимок API на момент съёмки;
контрактный тест закрывает разрыв между снимком и реальностью.

Проверяется **форма**: код ответа, наличие запрошенных полей, параллельность
колоночных массивов `daily`, декодируемость в DTO приложения. **Значения не
проверяются никогда** — погода меняется, и тест, ждущий `24.6`, будет падать
каждый час.

Запуск руками, при наличии сети:

    set -o pipefail && xcodebuild -scheme Meteo \
      -destination 'platform=iOS Simulator,name=iPhone 17,arch=arm64' \
      -derivedDataPath .build/DerivedData \
      -testPlan Contract -parallel-testing-enabled NO test | xcbeautify

Красный контрактный тест означает **изменение чужого API, а не баг в коде**.
Это написано в сообщении каждой проверки — чтобы человек, увидевший красное
через полгода, понял с первой строки. Чинится правкой DTO и маппера в `Data`.

Включаются переменной `CONTRACT`, которую задаёт только план `Contract`.
В `Fast` и `Full` наборы помечаются пропущенными.

Отобрать в плане одни лишь контрактные тесты **нельзя**, и причина точная:
`skippedTests` и `selectedTests` в плане работают **только для XCTest**.
Проверено в обе стороны — восемь форм идентификатора для наборов Swift Testing
не дали ничего, а `skippedTests` для XCTest-класса сработал с первого раза.

Это стоит помнить: прогон с отбором, который ничего не выбрал, **молча выполняет
ноль тестов и рапортует успех**. Самый опасный вид зелёного — увидев его,
проверяйте `Executed N tests`, а не только итоговую строку.

Поэтому `Contract` гоняет заодно юнит и интеграционные. Изолировать его можно
было бы, переписав контрактные тесты на XCTest, — намеренно не сделано:
полсекунды офлайновых тестов дешевле, чем два фреймворка в одном таргете
ради косметики.
...
| `Contract.xctestplan` | плюс контрактные, против живого API | руками, когда нужно свериться с API |
